...
### 2.1 The Toolbox
All evolved tools are stored in the `.pi/extensions/` directory with a specific naming pattern:
- `.pi/extensions/forge-<name>.ts`: The executable TypeScript logic.
- `.pi/extensions/forge-<name>.json`: Metadata, including the tool's description and TypeBox parameters schema.
- `.pi/extensions/forge-registry.json`: A central manifest for fast tool discovery during the `before_agent_start` hook.
...
- **Process**:
    ...
    2. Writes `.ts` and `.json` files to `.pi/extensions/`.
