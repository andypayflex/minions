 ...
 * if specified.
 *
 * Usage: pi -e .pi/extensions/system-select.ts -e .pi/extensions/minimal.ts
 */
...
import { listMarkdownFilesRecursive } from "./global-pi-paths.ts";
...
function scanAgents(dir: string, source: string): AgentDef[] {
	...
	try {
		for (const file of listMarkdownFilesRecursive(dir)) {
